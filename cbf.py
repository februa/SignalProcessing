...
class PlotAPI_BL(PlotAPI):
    def draw_graph(self, ax, signal: np.ndarray, N: int):
        nBeam = signal.shape[0]
        
        # 横軸：角度、縦軸：レベル表示の図        
        x_axis = np.arccos(np.linspace(1,-1,nBeam)) * 180 / pi
...
class ArraySignal(SignalType):
    def __init__(self, pos, nBeam, c, fs, N):
        super().__init__()
        self.count = 0
        self.pos = pos
        self.nBeam = nBeam
    ...
    def get_stearing_vector(self):
        theta = np.arccos(np.linspace(1,-1,nBeam))
        phi = np.zeros(nBeam)
        s = np.array([cos(phi) * cos(theta),
                      cos(phi) * sin(theta),
                      sin(phi)])
        delay = self.pos.dot(s)/self.c

        st_vec = np.zeros([self.nCh, self.nBeam, self.N//2],dtype=np.complex128)
        
        dF = fs//N
        for f_bin in range(self.N//2):
            f = f_bin * dF
            for beam in range(self.nBeam):
                st_vec[:, beam, f_bin] = exp(-1j*2*pi*f*delay[:,beam]).conj()
    ...
    def _make_delay_filter(self, theta, phi):
        ...
        f = np.linspace(0, self.fs//2, self.N//2, endpoint=False)
...
if __name__ == '__main__':
    ...
    x = -1*np.arange(nCh)*d
    pos[:,0] = x - x.mean(0)
    linearray = ArraySignal(pos, nBeam, c, fs, N)

    # 信号1：狭帯域信号
    f1 = 256
    rms1 = SL(0.0)
    tau1 = 0.0
    s1 = NarrowBand(f1, rms1, fs, N)

    # 信号2
    f2 = 256
    rms2 = SL(-5.0)
    tau1 = 0.0
    s2 = NarrowBand(f2, rms2, fs, N)
    
    # 白色雑音   
    rms_nl= NL(-25,fs)
    n1 = Noise(rms_nl, fs, N, nCh)
    
    linearray.add(s1, 60, 0)
    linearray.add(s2, 90, 0)
    linearray.add(n1)

    # ステアリングベクトル
    norm_func = lambda func, M: func(M) / func(M).sum()
    st_vec = linearray.get_stearing_vector()
    norm = norm_func(np.ones, nCh)
    
    cbf = np.zeros([int(fs*T/N), nBeam, N//2], dtype=np.complex128)

    for block in range(floor(fs*T/N)):
        y = linearray.generate()
        ...
        for b in range(int(N/2)):
            b_Y2 = Y[:,b]            
            cbf[block, :, b] = st_vec[:,:,b].T @ (b_Y2 * norm)

    fig, (ax1) = plt.subplots(1,1)
    plot_api = PlotAPI_BL()                       
    plot_api.draw_graph(ax1, cbf[block,:,int(f1/dF)], N)
    plt.show()
